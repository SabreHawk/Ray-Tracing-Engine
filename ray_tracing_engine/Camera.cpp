...
Ray Camera::gen_ray(double _u, double _v) {
    ...
    Ray tmp_r(origin + offset, low_left_corner + _u * horizontal_vec + _v * vertical_vec - origin - offset);
    return {tmp_r};
}

Camera::Camera(const Vector3 &_look_from, const Vector3 &_look_at, const Vector3 &_view_up, const double &_vfov,
               const double &_aspect, const double &_aperture, const double &_focus_dis) {
