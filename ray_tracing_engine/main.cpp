...
void render1(const char *_file_name);
...
Vector3 look_from(600, 1200, 600);
Vector3 look_at(0, 0, 0);
float focus_dis = (look_from - look_at).length();
Camera tmp_camera(look_from, look_at, Vector3(0, 1, 0), 30, float(width) / float(height), 0, 10, 0, 0);
...
int main()
{
    ...
    render1("/home/zhiquan/Git-Repository/Ray-Tracing-Engine/ray_tracing_engine/2.txt");
    return 0;
}
    ...
    for (int a = -11; a < 11; ++a)
    {
        for (int b = -11; b < 11; ++b)
        {
            ...
            if ((tmp_center - Vector3(4, 0.2, 0)).length() > 0.9)
            {
                ...
                {
                    Sphere *tmp_sphere = new Sphere(tmp_center, 0.2, new Lambertian(Vector3(drand48() * drand48(), drand48() * drand48(), drand48() * drand48())));
                ...
                {
                    tmp_scene.addObject(new Sphere(tmp_center, 0.2, new Metal(Vector3(0.5 * (1 + drand48()), 0.5 * (1 + drand48()), 0.5 * (1 + drand48())), 0.5 * drand48())));
                }
                else
                {
    ...
    if (in_stream)
    {
        ...
        for (int frame_index = 0; frame_index < 265; ++frame_index)
        {
            ...
            for (int p_index = 0; p_index < p_num; ++p_index)
            {
                ...
                tmp_scene.addObject(new Sphere(Vector3(tmp_vec[0], tmp_vec[1], tmp_vec[2]), 10, new Metal(Vector3(0.5 * (1 + drand48()), 0.5 * (1 + drand48()), 0.5 * (1 + drand48())), 0.5 * drand48())));
            }
            ...
            for (int i = 0; i < height; ++i)
            {
                if (i % 2 == 0)
                {
                    ...
                    printf("\tEstimated Time : %.2f min",
                           (double)(height - i) / i * (double)duration.count() / std::chrono::microseconds::period::den / 60);
                }
            ...
            tmp_scene.clear();
            std::cout << flush << '\r' << frame_index + 1 << "/" << f_num << " Picture Generated Successfully " << endl;
        }
    }
    else
    {
...
void read_fluid(const char *_file_name)
{
    ...
    if (in_stream)
    {
        ...
        std::stringstream s_stream(tmp_line);
        s_stream >> fluid_dataset_name;
        std::cout << fluid_dataset_name << endl
                  << endl;
        //read fluid parameters
        getline(in_stream, tmp_line);
        std::cout << "tmp " << tmp_line << endl;
        ...
        for (int i = 0; i < 1; ++i)
        {
            ...
            for (int j = 0; j < p_num; ++j)
            {
                ...
                std::cout << Vector3(tmp_vec[0], tmp_vec[1], tmp_vec[2]) << endl;
                tmp_scene.addObject(new Sphere(Vector3(tmp_vec[0], tmp_vec[1], tmp_vec[2]), 16, new Metal(Vector3(0.5 * (1 + drand48()), 0.5 * (1 + drand48()), 0.5 * (1 + drand48())), 0.5 * drand48())));
            }
        }
    }
    else
